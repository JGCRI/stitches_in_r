...
1. For each ESM-experiment-ensemble member’s monthly temperature data 
...
x <- read.csv(paste0(timeseries_dir,'global_tas_allesms_timeseries_2015_2100.csv'), 
              stringsAsFactors = F) %>% mutate(region = 'Global')
...
# IPCC regions
```{r}
y <-  read.csv(paste0(timeseries_dir,'IPCC_land_regions_tas_ACCESS-ESM1-5_timeseries_1980_2099.csv'), 
              stringsAsFactors = F) %>% mutate(region = acronym)
...
conus_tas <- read.csv(paste0(timeseries_dir,'CONUS_tas_allesms_timeseries_2015_2100.csv'), 
                                                          stringsAsFactors = F) %>%
  mutate(region='CONUS')
...
# CONUS pr pct change
```{r, echo = FALSE, message = FALSE}
conus_pr <-  read.csv(paste0(timeseries_dir,'CONUS_pr_allesms_timeseries_2015_2100.csv'),
                      stringsAsFactors = F)  %>%
  mutate(region='CONUS')
...
```{r, echo = FALSE, message = FALSE}
sw_tas <- read.csv(paste0(timeseries_dir,'southwest_tas_allesms_timeseries_1850_2100.csv'), 
                   stringsAsFactors = F) %>%
  filter(year >=1995) %>%
  mutate(region='sw')
...
```{r, echo = FALSE, message = FALSE}
sw_pr <- read.csv(paste0(timeseries_dir,'southwest_pr_allesms_timeseries_1850_2100.csv'), 
                  stringsAsFactors = F) %>%
  filter(year >=1995) %>%
  mutate(region='sw')
