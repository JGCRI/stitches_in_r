...
esm_labels <- read.csv(paste0('/Users/snyd535/Documents/GitHub/stitches_in_r/R/inst/shinyApp/python_curation/extracted_timeseries/global_tas_allesms.csv'), stringsAsFactors = FALSE) %>%
  select(esm) %>% distinct %>% 
  mutate(plotesm = paste0(letters[as.integer(row.names(.))], '.', esm),
         ECS_order = as.integer(row.names(.)))
print(esm_labels)

process_time_series <- function(time_series_df, 
  ...
  processed_groups <- lapply(grouped, FUN = function(run_df){
    ...
    run_df %>%
        ...
        left_join(run_df %>%
                      ...
                      ungroup,
                  by = c('esm', 'experiment', 'ensemble', 'variable', 'region')) %>% 
...
prep_esm_TP_data_ipcc <- function(esmname, metric_write_dir = metrics_write_dir){
    ...
    if(esmname %in% c(esm_labels$esm )){
  ...
  pr_timeseries <- read.csv(paste0(timeseries_dir, 'IPCC_land_regions_pr_',
                                   esmname,'_timeseries_1980_2099.csv'),
  ...
  if (nrow(region_timeseries)>0 & nrow(pr_timeseries)>0){
      ...
      region_tas_summary[[2]] %>% 
        ...
        rename(tas_change_2080_2099 = change_2080_2099,
               tas_change_2040_2059 = change_2040_2059) %>%
        ...
        rename(pr_pct_change_2080_2099 = pct_change_2080_2099,
               pr_pct_change_2040_2059 = pct_change_2040_2059) ->
        region_summary_ind
      ...
      region_tas_summary[[1]] %>% 
       select(esm, experiment,  region, iasd, change_2080_2099, change_2040_2059) %>%
        rename(tas_change_2080_2099 = change_2080_2099,
               tas_change_2040_2059 = change_2040_2059) %>%
        left_join(region_pr_summary[[1]] %>%
                    select(esm, experiment,  region, pct_change_2080_2099, pct_change_2040_2059) ,
