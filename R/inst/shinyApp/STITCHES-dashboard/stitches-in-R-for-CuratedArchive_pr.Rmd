...
# python - use the geopandas shape files to set region to calculate on
...
# create contiguous US polygon for clipping CMIP5 data
world = gpd.read_file(gpd.datasets.get_path("naturalearth_lowres"))
US=world[world['name']=='United States of America']  # conus, AK, HI, PR
conus_poly = Polygon([(-126, 24), (-126, 50), (-66, 50), (-66, 24), (-126, 24)]) # conus box
# conus=gpd.clip(US,conus_poly) #cut out AK, HI, PR
# conus.geometry
```
  ...
  for exp in exps[0:2]:
    ...
    if not filelist.empty:
      ...
      for j in range(len(r.map_info)):
        ...
        pr_holder = pr_holder.append(pd.DataFrame({'esm':[esmname],
        'experiment':[exp],
        'map_lon': [map_lon],
        'map_lat': [map_lat],
        'data_lon': [extracted_cell.lon.values],
        'data_lat': [extracted_cell.lat.values],
        'ens_pr': [grid_val],
        'area_weight':[weight]})
        ).copy()
    ...
    pr_holder[esmname+'~'+exp] = ensemble_ds
    print(pr_holder)
    #del(ensemble_ds)
